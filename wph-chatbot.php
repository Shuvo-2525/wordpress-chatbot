...
function wph_chatbot_dynamic_css() {
     ...
     // Sanitize the color just in case
     $theme_color = sanitize_hex_color($theme_color) ?: '#00665E';
 ...
 add_action('admin_init', 'redirect_add_new_wph_entries');
    ...
    function remove_add_new_button_for_wph_entries() {
     ...
     if ($pagenow === 'edit.php' && isset($_GET['post_type']) && $_GET['post_type'] === 'wph_entries') {
         // Remove the "Add New" button next to the title
...
function wph_entries_row_actions($actions, $post) {
     if ($post->post_type === 'wph_entries') {
         ...
         // Keep 'trash' and 'view' to modify them
         
         // Customize the "View" link to point to the edit screen (acting as view)
         if (isset($actions['view'])) {
             $edit_link = get_edit_post_link($post->ID);
             $actions['view'] = '<a href="' . esc_url($edit_link) . '" aria-label="' . esc_attr__( 'View details for ') . $post->post_title . '">' . __('View') . '</a>';
         }
         
         // Ensure the "Trash" link is correct
         if (isset($actions['trash'])) {
             // FIX: Use $post->ID instead of potentially undefined $post_id
             $trash_link = get_delete_post_link($post->ID); 
             if ($trash_link) {
                 $actions['trash'] = '<a href="' . esc_url($trash_link) . '" class="submitdelete" aria-label="' . esc_attr__( 'Move ') . $post->post_title . __(' to the Trash') . '">' . __('Trash') . '</a>';
             } else {
                 unset($actions['trash']); // Remove if link generation fails
             }
         }
     }
     return $actions;
...
function add_inline_css_to_post_editor_wph() {
     ...
     if ($screen && $screen->id === 'wph_entries' && $screen->base === 'post') {
         wp_enqueue_style('wp-editor'); // Make sure editor styles are loaded
         $custom_css = "
         /* Hide the title input field */
         .post-type-wph_entries div#titlediv {
             display: none;
         }
         /* Move the meta box to the top */
         .post-type-wph_entries div#post-body {
             display: flex;
             flex-direction: column-reverse;
             margin: 0 !important; /* Overrides WP core styles if needed */
         }
         ";
